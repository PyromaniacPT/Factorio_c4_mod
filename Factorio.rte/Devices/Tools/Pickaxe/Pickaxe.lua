function Create(self)
	self.origStanceOffset = Vector(4, 6);
	...
	self.hitSound = CreateSoundContainer("Factorio Pickaxe Hit", "Factorio.rte");
	--How much the Pickaxe tilts when firing
	self.angleSize = 1.0;
...
function Update(self)
	...
	if parent and IsActor(parent) then
		...
		local resource = parent:GetNumberValue("FactorioPickaxeResource");
		...
		if self.FiredFrame then
			...
			for i = 1, particleCount do
				...
				local dig = CreateMOPixel("Particle Factorio Pickaxe 2", "Factorio.rte");
				dig.Pos = self.Pos;
						...
						for _, terrainMaterial in pairs(self.suitableMaterials) do
							if material == terrainMaterial then
								...
								if hits > rayCount * 0.5 then
									parent:SetNumberValue("FactorioPickaxeResource", resource + 1);
...
function OnPieMenu(item)
	if item and IsHDFirearm(item) and item.PresetName == "Pickaxe" then
		item = ToHDFirearm(item);
		local parent = item:GetRootParent();
		if parent and IsActor(parent) then
			local resource = ToActor(parent):GetNumberValue("FactorioPickaxeResource");
			ToGameActivity(ActivityMan:GetActivity()):RemovePieMenuSlice("Fill Sandbag", "FactorioCreateSandbag");
			ToGameActivity(ActivityMan:GetActivity()):AddPieMenuSlice("Fill Sandbag", "FactorioCreateSandbag", Slice.RIGHT, resource >= 10);
		end
	end
end